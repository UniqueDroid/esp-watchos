...
constexpr base::Manager::Data STYLESHEET_410_502_DARK_CORE_MANAGER_DATA = {
    .app = {
        /* This device has no PSRAM and very little free heap (seen as low as
         * ~4KB). Keeping up to 3 apps resident in the background at once was
         * a major contributor to running out of memory while a 4th app's UI
         * was being built. Only the foreground app stays resident now; the
         * previous one is fully closed (and its widgets freed) the moment a
         * new app is opened. */
        .max_running_num = 1,
    },
    .flags = {
        /* Taking a live screen snapshot for the recents/task-switcher screen
         * allocates a full-screen render layer buffer; this device has no
         * PSRAM and only ~30-40KB of free heap, which isn't enough to do
         * that reliably while a content-heavy app screen (e.g. a popup with
         * a keyboard) is showing - it hangs in the allocator instead of
         * failing cleanly. Recents screen falls back to each app's launcher
         * icon instead, which is free. */
